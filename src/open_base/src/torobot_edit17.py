...
def rumus():
    ...
    thetas1 = math.degrees(atan(((yk-yj)/(xk-xj)))) #HASIL DALAM BENTUK RADIAN
    thetas1rad = atan(((yk-yj)/(xk-xj))) #HASIL DALAM BENTUK RADIAN
    thetaf1 = math.degrees(atan(((yk-yl)/(xk-xl))))#HASIL DALAM BENTUK RADIAN
...
def open_base3():
    ...
    if (mulai == 1 and x3 <= xs) :
        print('menuju xs dan ys')
        ...
        vel.generic.type = 0
        vel.generic.frame = 3
        vel.generic.target.x = xs
        vel.generic.target.y = ys
        vel.generic.target.theta = thetas
        movePub1.publish(vel)
        if (x3 >= (xs-0.02) and y3 >= (ys-0.02)) :
            mulai = 0
            varC =1
...
def open_base33():
    ...
    if(varE == 1):
        ...
        vel.generic.type = 0
        vel.generic.frame = 3
        vel.generic.target.x = xf
        vel.generic.target.y = yf
        vel.generic.target.theta = thetaf
        movePub1.publish(vel)
        if (x3 >= (xf-0.02) and y3 >= (yf-0.02)) :
            varF = 1
            varG = 0 
            varE = 0
            varD = 0
...
def open_base34():
    ...
    if(varF == 1):
        ...
        vel.generic.type = 0
        vel.generic.frame = 3
        vel.generic.target.x = xs1
        vel.generic.target.y = ys1
        vel.generic.target.theta = thetas1
        movePub1.publish(vel)
        if (x3 >= (xs1-0.03) and y3 >= (ys1-0.03)) :
            varG = 1
            varF = 0
            varE = 0
            varD = 0
...
def open_base35():
    global x3,y3, varC, varD, mulai, varE, varF,xf,yf,xs,ys,Xrf,Yrf,Yrs,Xrs,thetas,thetaf,varG,varH,varI,varJ,xl,yl
    if (varG == 1) :
        ...
        vel.generic.target.theta = 0
        movePub1.publish(vel)
        if (x3 >= (Xrs1-0.03) and y3 >= (Yrs1-0.03)) :
            varH = 1
            varG = 0
            varF = 0
            varE = 0
...
def open_base36():
    ...
    if (varH == 1) :
        ...
        vel.generic.target.theta = 0
        movePub1.publish(vel)
        if (x3 >= (Xrf1-0.03) and y3 >= (Yrf1-0.03) ) :
            varI = 1
            varH = 0
            varG = 0
            varF = 0
...
def open_base37():
    global x3,y3, varC, varD, mulai,xf1,yf1,xs1,ys1, varE, varF,xf,yf,xs,ys,Xrf,Yrf,Yrs,Xrs,thetas,thetaf,varG,varH,varI,varJ,xl,yl
    if (varI == 1) :
        print('menuju xf1 dan yf1')
        movePub1 = rospy.Publisher('/open_base3/command', Movement, queue_size=1)
        rate = rospy.Rate(1)
        vel = Movement()
        vel.movement = 1
        vel.generic.type = 0
        vel.generic.frame = 3
        vel.generic.target.x = xf1
        vel.generic.target.y = yf1
        vel.generic.target.theta = thetaf1
        movePub1.publish(vel)
        if (x3 >= (xf1-0.03) and y3 >= (yf1-0.03) ) :
            varJ = 1
            varH = 0
            varG = 0
            varF = 0
