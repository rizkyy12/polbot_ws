...
def dari2ke3(x_goal1, y_goal1):
    ...
    velocity_publisher1 = rospy.Publisher('/open_base2/command', Movement, queue_size=1)
    rate = rospy.Rate(1)
    vel = Movement()
    vel.movement = 1
    vel.generic.type = 2
	...
	#linear_speed = distance * K_linear
        ...
        vel.generic.target.x = abs(K_linear*ex1*math.cos(desired_angle_goal)) - (K_linear*ey1*math.sin(desired_angle_goal))
        vel.generic.target.y = (K_linear*ex1*math.sin(desired_angle_goal)) + (K_linear*ey1*math.cos(desired_angle_goal))
        vel.generic.target.theta = angular_speed
...
def dari3ke1(x1a, y1a):
    ...
    velocity_publisher2 = rospy.Publisher('/open_base3/command', Movement, queue_size=1)
    rate = rospy.Rate(1)
    vel = Movement()
    vel.movement = 1
    vel.generic.type = 2
	...
	#linear_speed = distance * K_linear	
        ...
        vel.generic.target.x = abs(K_linear*ex2*math.cos(desired_angle_goal)) - (K_linear*ey2*math.sin(desired_angle_goal))
        vel.generic.target.y = (K_linear*ey2*math.sin(desired_angle_goal)) + (K_linear*ey2*math.cos(desired_angle_goal))
        vel.generic.target.theta = angular_speed
...
if __name__ == '__main__':
	...
	#Robot 2 ke Robot 3
        velocity_publisher1 = rospy.Publisher('/open_base2/command', Movement, queue_size=1)   
        odom1_subscriber = rospy.Subscriber("/open_base2/pose/world", Odometry, posisi2)
        time.sleep(1)
        odim1_subscriber = rospy.Subscriber('/open_base3/pose/world', Odometry, posisi3)
        time.sleep(1)
	...
	#Robot 3 ke Robot 1
        velocity_publisher2 = rospy.Publisher('/open_base3/command', Movement, queue_size=1)   
        odom2_subscriber = rospy.Subscriber("/open_base3/pose/world", Odometry, posisi4)
        time.sleep(1)
        odim2_subscriber = rospy.Subscriber('/open_base1/pose/world', Odometry, posisiawal)
        time.sleep(1)
        dari3ke1(x1a, y1a)
        time.sleep(1)
