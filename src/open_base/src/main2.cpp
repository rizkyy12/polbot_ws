...
int main(int argc, char **argv) {
    ...
    {
        double parameter;
        if (!node.getParam("parameter2/wheel/radius", parameter)) {
            ROS_ERROR("Could not get wheel radius from parameter server.");
            return -1;
        }
        radius = parameter;
        if (!node.getParam("parameter2/stop/distance", parameter)) {
            parameter = 0.001;
        }
        stopDistance = parameter;
        if (!node.getParam("parameter2/stop/angle", parameter)) {
            parameter = pi30;
        }
        stopAngle = parameter;
        if (!node.getParam("parameter2/max/v", parameter)) {
            parameter = 1;
        }
        v_max = parameter;
        if (!node.getParam("parameter2/max/omega", parameter)) {
            parameter = 50;
        }
        omega_max = parameter;
        if (!node.getParam("parameter2/control/P", parameter)) {
            parameter = 5;
        }
        P = parameter;
        if (!node.getParam("parameter2/control/I", parameter)) {
            parameter = 0.0005;
        }
        I = parameter;
    }
